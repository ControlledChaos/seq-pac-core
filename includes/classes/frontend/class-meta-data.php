...
namespace SPR_Core\Classes\Front\Meta;
...
class Meta_Data {
	...
	public function description() {
		...
		$search = wp_strip_all_tags(
			sprintf(
				'%s %s',
				__( 'Showing results for', 'spr-core' ),
				get_search_query()
			)
		);
		...
		if ( ! empty( $site_description ) && ( is_front_page() || is_home() ) ) {
		...
		} elseif ( is_post_type_archive( 'featured' ) ) {
			$description = sprintf(
				'%s %s',
				__( 'Real estate listings featured by', 'spr-core' ),
				get_bloginfo( 'name' )
			);
			...
			$description = sprintf(
				'%s %s',
				__( 'Real estate listings by location', 'spr-core' ),
				get_bloginfo( 'name' )
			);
			...
			$description = sprintf(
				'%s %s',
				__( 'Rental properties managed by', 'spr-core' ),
				get_bloginfo( 'name' )
			);
			...
			$description = sprintf(
				'%s %s',
				__( 'Real estate listing featured by', 'spr-core' ),
				get_bloginfo( 'name' )
			);

		} elseif ( is_singular( 'location' ) ) {
			$description = __( 'Real estate listings by location', 'spr-core' );

		} elseif ( is_singular( 'rental' ) ) {
			$description = sprintf(
				'%s %s',
				__( 'Rental property managed by', 'spr-core' ),
				get_bloginfo( 'name' )
			);

		} elseif (
			is_singular() &&
			post_type_supports( get_post_type( $post_id ), 'excerpt' ) &&
			has_excerpt() )
		{
			...
			! empty( $auto_excerpt ) )
		{
			...
			$description = __( '404 Error: Not Found', 'spr-core' );
	...
	public function copyright() {
		...
		$copy = sprintf(
			'&copy; %s %s %s %s',
			__( 'Copyright', 'spr-core' ),
			get_the_time( 'Y' ),
			get_bloginfo( 'name' ),
			__( '. All rights reserved.', 'spr-core' )
		);
